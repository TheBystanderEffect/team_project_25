    ...
    static magic(diagram) {
        ...
        diagram.layers.forEach((layer, index) => {
            ...
            if (!layer.graphicElement) {
                var templayerptr = new LayerView(layer, 0, 0, -Config.firstLayerOffset - Config.layerOffset * index, dynamicLayerWidth, //Config.layerWidth,
                dynamicLayerHeight //Config.layerHeight
                );
                ...
                templayerptr = layer.graphicElement;
            ...
            layer.lifelines.forEach((lifeline, index) => {
                if (!lifeline.graphicElement) {
                    ...
                    lifeline, -10, //templifelineptr.source.x-10,
                    5, //templifelineptr.source.y+5,
                    0, //templifelineptr.source.z,
                    lifeline.name, Config.lifelineTextSize);
                    var templifelineptr = new LifelineView(lifeline, templayerptr.source.x + Config.firstLifelineOffsetX + Config.lifelineOffsetX * index, templayerptr.source.y - Config.lifelineOffsetY, 0, templayerptr.height - Config.lifelineOffsetY);
                    ...
                    // console.log("UPDATE");
                    templifelineptr = lifeline.graphicElement;
                    templifelineptr.position.set(templayerptr.source.x + Config.firstLifelineOffsetX + Config.lifelineOffsetX * index, templayerptr.source.y - Config.lifelineOffsetY, 0);
            ...
            layer.messages.forEach((message, index) => {
                if (!message.graphicElement) {
                    var start = message.start.at.graphicElement.source;
                    var end = message.end.at.graphicElement.source;
                    var tempTextPtr = new TextView(//TODO add offsets and make it not retarded and dependant on mesh
                    message, -10, 5, 0, message.name, Config.messageTextSize);
                    var tempMessagePtr = new MessageView(message, start.x, start.y - Config.firstMessageOffset - Config.messageOffset * index, 0, end.x, end.y - Config.firstMessageOffset - Config.messageOffset * index, 0);
                    ...
                    tempMessagePtr = message.graphicElement;
                    var start = message.start.at.graphicElement.position;
                    var end = message.end.at.graphicElement.position;
                    ...
                    tempMessagePtr.center = new Vector3((start.x + end.x) / 2, start.y - Config.firstMessageOffset - Config.messageOffset * index, (start.z + end.z) / 2);
